parallelPCA <- function(x, max.rank=100, ..., niters=50, threshold=0.1, 
  BSPARAM=ExactParam(), BPPARAM=SerialParam()) 
{
  original <- pca(y, rank=max.rank, ..., BSPARAM=BSPARAM)
  ...
  permuted <- bpmapply(FUN=.parallel_PA,
    ...
    MoreArgs=list(y=y, ..., max.rank=max.rank, BSPARAM=BSPARAM), 
...
.parallel_PA <- function(y, max.rank, ..., seed, stream, BSPARAM)
# Function for use in bplapply, defined here to automatically 
# take advantage of the namespace when using snowParam. 
{
  re.y <- shuffle_matrix(y, seed, stream)
  out <- pca(re.y, rank=max.rank, ..., BSPARAM=BSPARAM)
  out$variance
}
