...
```{r ex1, warning = FALSE, fig.height = 7, fig.width = 20, fig.cap = 'Figure 1: A scree plot to show the proportion of explained variance by PC'}
...
general users, a biplot is a simple representation of samples in a 2-dimensional space:

```{r ex2a, fig.height = 6, fig.width = 7, fig.cap = 'Figure 2a: A bi-plot of PC1 versus PC2'}

  biplot(p)

```

However, the original definition of a biplot by Gabriel KR [@Gabriel] is a plot that plots
both variables and observatinos (samples) in the same space. The variables are indicated by
arrows drawn from the origin, which indicate their 'weight' in different directions. We touch
on this later via the *plotLoadings* function.

```{r ex2b, fig.height = 6, fig.width = 7, fig.cap = 'Figure 2b: A bi-plot of PC1 versus PC2'}

  biplot(p, showLoadings = TRUE, lab = NULL)

```

One of the probes pointing downward is *205225_at*, which targets the *ESR1* gene. This is already
a useful validation, as the oestrogen receptor, which is in part encoded by *ESR1*, is strongly
represented by PC2 (y-axis), with negative-to-positive receptor status going from top-to-bottom.
...
The rotated data that represents the observatinos / samples is stored in *rotated*,
...
First, let's sort out the gene annotation by mapping the probe IDs to gene symbols.
The array used for this study was the Affymetrix U133a, so let's use the *hgu133a.db*
...
```{r ex9, message = FALSE, eval = FALSE}
...
```{r ex10a, message = FALSE, fig.height = 5.5, fig.width = 7, fig.cap = 'Figure 10a: removing labels, modifying line types, removing gridlines, and increasing point size in a bi-plot'}
...
```{r ex10b, message = FALSE, fig.height = 5.5, fig.width = 10, fig.cap = 'Figure 10b: removing labels, modifying line types, removing gridlines, and increasing point size in a bi-plot'}
...
```{r ex11a, fig.height = 5.5, fig.width = 7, fig.cap = 'Figure 11a: colouring by a continuous variable and plotting other PCs in a bi-plot'}
...
```{r ex13, fig.height = 5.5, fig.width = 6, fig.cap = 'Figure 13: arranging a pairs plot horizontally'}
...
```{r ex14, fig.height = 5.5, fig.width = 7, fig.cap = 'Figure 14: modifying cut-off for labeling in a loadings plot'}
...
this results is also verified in these.
...
```{r ex15, fig.height = 7, fig.width = 9, fig.cap = 'Figure 15: plotting absolute component loadings'}
